  ...
  {}
    ...
    def find_schema(self, payload=None, name=None, to=None):
        ...
        for schema in self.messages.values():
            ...
            #remove signed parameters
            parameter_payload = {}
            for parameter in payload:
                if not parameter.endswith("_signed"):
                    parameter_payload[parameter] = payload[parameter]
            
            # find schema with exactly the same parameters (except nils, which should not be bound)
            if (
                not set(schema.ins)
                .union(schema.outs)
                .symmetric_difference(parameter_payload.keys())
            ):
                return schema
    ...
    #HELPER FUNCTION
    def deep_search_for_origin(self, message, parameter):
        protocol_dict = self.to_dict()
        
        print("DEEP SEARCH CALLED FOR M:{} and P:{}".format(message, parameter))
        
        print("TO DO")
        """
        for message_nested in protocol_dict["messages"]:
            if protocol_dict["messages"][message_nested]["to"] == protocol_dict["messages"][message]["from"] and parameter in protocol_dict["messages"][message_nested]["outs"]:
                self.parameter_origins_dict[(message, parameter)] = protocol_dict["messages"][message_nested]["from"]
                continue
            """
    
    
    #This function maps parameters in messages to their originating entity
    @property
    def parameter_origins(self):        
        if len(self.parameter_origins_dict) == 0:
            protocol_dict = self.to_dict()
            
            
            
            
            #identify parameters with more than one origin
            outputting_entities = {}
            #identify all "out" parameter mentions
            for parameter in self.all_parameters:
                outputting_entities[parameter] = []
                for message in protocol_dict["messages"]:#loop through messages
                    if parameter in protocol_dict["messages"][message]["outs"]:#if parameter is adorned out
                        outputting_entities[parameter].append(protocol_dict["messages"][message]["from"])
            ...
            #populate parameter_origins_dict
            for message in protocol_dict["messages"]:#loop through messages
                for parameter in protocol_dict["messages"][message]["parameters"]:#loop through parameters in messages
                    if len(outputting_entities[parameter]) == 1:#if only one outputting entity
                        self.parameter_origins_dict[(message, parameter)] = outputting_entities[parameter][0]
                    elif parameter in protocol_dict["messages"][message]["outs"]:#if parameter is adorned out
                        self.parameter_origins_dict[(message, parameter)] = protocol_dict["messages"][message]["from"]
                    else:
                        self.parameter_origins_dict[(message, parameter)] = self.deep_search_for_origin(message, parameter)
                        
            
        return self.parameter_origins_dict
...
class Message(Protocol):
    ...
    def match(self, **params):
        ...
        for c in contexts:
            if (
                h.check_outs(self, c)
                and all(p in c.bindings for p in self.ins)
                and not any(p in c.bindings for p in self.nils)
            ):
                # use self.__call__ injected by adapter
                candidates.add(self(**c.bindings))
        return candidates

...
logger = logging.getLogger("bspl.adapter.emitter")
...
class Emitter:
    ...
    #This method calls upon the encryption handler and loads required keys in 
    #the encryption handler using the config
    def load_key_handler(self, component_name, config):
        self.encryptor.get_keys(component_name, config)
    ...
    async def bulk_send(self, messages):
        ...
        for m in messages:
            ...
            if (m.dest, m.schema.recipient.name) in packets:
                if len(packets[(m.dest, m.schema.recipient.name)]) + len(data) + 3 > self.mtu:
                    if len(packets[(m.dest, m.schema.recipient.name)]) + 2 > self.mtu:
                        raise Exception(
                            f"Message is too long to fit in a single packet: {packets[m.dest]}"
                        )
                    else:
                        # send what we have so far and reset
                        packet = b"[" + packets[(m.dest, m.schema.recipient.name)] + b"]"
                        self.transmit(packet, m.dest)
                        packets[(m.dest, m.schema.recipient.name)] = data
                else:
                    # add one more message
                    packets[(m.dest, m.schema.recipient.name)] += b"," + data
            else:
                packets[(m.dest, m.schema.recipient.name)] = data
    ...
    def transmit(self, packet, dest):
        ...
        #Encrypt the packet
        encrypted_packet = self.encryption_handler.encrypt(dest[1], packet)
        
        self.stats["bytes"] += len(encrypted_packet)
        self.stats["packets"] += 1
        self.socket.sendto(encrypted_packet, dest[0])
